...
def main() -> None:
    p = argparse.ArgumentParser(description="Check Nano Banana-compatible image model connectivity using .env config.")
    ...
    try:
        ...
        fatal(f"Nano Banana 兼容图片模型连通性检查失败：{exc}")

    dotenv = str(cfg.dotenv_path) if cfg.dotenv_path is not None else "(not found)"
    info(f"OK: dotenv={dotenv}, provider={cfg.provider}, base_url={cfg.base_url}, model={cfg.model}")
